...
def Interpolate(qlist,vmax,amax,vcoeflist=None,acoeflist=None,tstart=0):
    npoints = len(qlist)
    assert(npoints>=3)
    if(vcoeflist is None):
        vcoeflist = ones(npoints-1)
    if(acoeflist is None):
...
def MakeTraj(x,qstart,qend,ndof,nwaypoints,nsamples,vmax,amax):
...
def ObjFunc(x,qstart,qend,ndof,nwaypoints,nsamples,weights,vmax,amax,trajref):
    traj = MakeTraj(x,qstart,qend,ndof,nwaypoints,nsamples,vmax,amax)
...
def CreateProgram(x, trajref, nwaypoints, filename, nextracols=0):
    ndof = trajref.dimension
    qlist = [trajref.Eval(0)]
    for i in range(nwaypoints):
        qlist.append(x[i*ndof:(i+1)*ndof])

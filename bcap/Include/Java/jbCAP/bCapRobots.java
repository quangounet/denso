...
public class bCapRobots {
	private int m_hr;
	private int m_iHandle;
	private bCapSocket m_bCapSock;
	private ArrayList<bCapRobot> m_robot;
	
	protected bCapRobots(int iHandle, bCapSocket sock)
	{
		m_hr = 0;
		m_iHandle = iHandle;
		m_bCapSock = sock;
		m_robot = new ArrayList<bCapRobot>();
	}
	
	public bCapRobot Item(int index)
	{
		bCapRobot robot = null;
		
		if(0 <= index && index < m_robot.size())
		{
			robot = m_robot.get(index);
		}
		
		return robot;
	}
	
	public bCapRobot Item(VARIANT index)
	{
		bCapRobot robot = null;
		...
		if(index.VariantGetType() == VARENUM.VT_BSTR)
		{
			...
			{
				if(((String)m_robot.get(i).get_Name().VariantGetObject()).equals((String) index.VariantGetObject()))
					robot = m_robot.get(i);
			}
		}
	...
	public bCapRobot Add(String strName, String strOption)
	{
		bCapPacket msg = new bCapPacket();
		msg.SetID(7); // Controller_GetRobot
		...
		{
			bCapRobot robot = new bCapRobot(strName, strOption, (Integer) msg.GetVariant(0).VariantGetObject(), m_bCapSock);
	...
	public void Remove(int index)
	{
		if(0 <= index && index < m_robot.size())
		{
			m_robot.get(index).Disconnect();
			m_hr = m_robot.get(index).HRESULT();
		}
		else
		{
			m_hr = -1;
	...
	public void Remove(VARIANT index)
	{
		...
		if(index.VariantGetType() == VARENUM.VT_BSTR)
		{
			for(int i = 0; i < m_robot.size(); i++)
			{
				if(((String) m_robot.get(i).get_Name().VariantGetObject()).equals((String) index.VariantGetObject()))
				{
					m_robot.get(i).Disconnect();
					m_hr = m_robot.get(i).HRESULT();
